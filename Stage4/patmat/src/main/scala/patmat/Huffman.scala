...
object Huffman {
  ...
  def findAndRemove(
                     ...
                     head: List[(Char, Int)],
                     tail: List[(Char, Int)]
                   ): ((Char, Int), List[(Char, Int)]) = {
    tail match {
      case x::rest  => if (x._1 == char) (x, head:::rest) else findAndRemove(char, head:::List(x), rest)
      case Nil      => ((char, 0), head)
    }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    trees match {
      case left :: right :: list => (makeCodeTree(left, right) :: list).sortBy(tree => weight(tree))
      case _ => trees
    }
  ...
  /**
    ...
    */
  def until[T<:CodeTree](
                singleton:List[T] => Boolean,
                combine:  List[T] => List[T])(tree: List[T]): List[T] =
    if (singleton(tree))
      tree
    else
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def lookup(tree: CodeTree)(c: Char): List[Bit] =
      tree match {
        case Leaf(_, _) => List()
  ...
  def codeBits2(table: CodeTable)(char: Char): List[Bit] =
